		...
		$Row = 'row'.$Index;
			$Form .= '<div class="rowWrapperForm">';
								...
								$Title = df_parseCamelCase($FieldSet['Name']);
                                                                ...
                                                                if(!empty($Element['Content']['_FieldTitle'][$FieldSet['Name']])){
                                                                    if(substr($FieldSet['Name'], 0, 2) == '__'){
                                                                        $Title = '<img src="'.WP_PLUGIN_URL.'/dbtoolkit/data_report/copy.png" width="16" height="16" align="absmiddle" /> '.$Element['Content']['_FieldTitle'][$FieldSet['Name']];
                                                                    }else{
								...
								if(substr($FieldSet['Name'],0,13) != '_SectionBreak' && substr($FieldSet['Name'],0,4) != '_tab'){
									...
									$Form .= '<div class="formportlet" id="formportlet_Field_'.$FieldSet['Name'].'"><div class="formportlet-header">'.$Title.'<input class="layOutform positioning" type="hidden" name="'.$FieldName.'" id="'.$FieldSet['Name'].'" value="'.$Row.'_'.$Column.'_'.$Field['Width'].'"/><span class="ui-icon ui-icon-close"></span></div></div>';
                                                                        ...
                                                                        if(substr($FieldSet['Name'],0,4) == '_tab'){
                                                                            ...
                                                                            //$Form .= '<div class="formportlet" id="formportlet_Field_'.$FieldSet['Name'].'"><div class="formportlet-header">'.$Title.'<input class="layOutform positioning" type="hidden" name="'.$FieldName.'" id="'.$FieldSet['Name'].'" value="'.$Row.'_'.$Column.'_'.$Field['Width'].'"/><span class="ui-icon ui-icon-close"></span></div></div>';
                                                                            $Form .= '<div style="padding: 3px;" class="list_row4 table_sorter tab formportlet ui-widget ui-widget-content ui-helper-clearfix ui-corner-all" id="tab_tab\'+number+\'"><div class="formportlet-header ui-widget-header ui-corner-all"><img align="absmiddle" class="OrderSorter" src="'.WP_PLUGIN_URL.'/dbtoolkit/data_report/arrow_out.png" style="-moz-user-select: none;"/>&nbsp;<input type="text" class="sectionTitle" name="Data[Content][_Tab]['.$FieldSet['Name'].'][Title]" value="'.$FieldName.'" /><span class="ui-icon ui-icon-close"></span></div><input class="layOutform positioning" type="hidden" name="'.$FieldSet['Name'].'" id="'.$FieldSet['Name'].'" value="'.$Row.'_'.$Column.'_'.$Field['Width'].'"/></div>';
                                                                        }else{
                                                                            //dump($Element['Content']['_SectionBreak'][$FieldSet['Name']]['Title']);
				...
				$Form .= ' <div style="width:10%; padding-top:12px; float:left;" class="formRow" id="'.$Row.'Control">';
					$Form .= '<img src="'.WP_PLUGIN_URL.'/dbtoolkit/data_report/cog.png" style="cursor:pointer;" width="16" height="16" onclick="formSetupColumns(\''.$Row.'\');" /><img src="'.WP_PLUGIN_URL.'/dbtoolkit/data_report/add.png" style="cursor:pointer;" width="16" height="16" onclick="formAddColumn(\''.$Row.'\');" /><img src="'.WP_PLUGIN_URL.'/dbtoolkit/data_report/delete.png" style="cursor:pointer;" width="16" height="16" onclick="formSubtractColumn(\''.$Row.'\');" />';
					if($Index > 1){
						$Form .= '<img src="'.WP_PLUGIN_URL.'/dbtoolkit/data_report/no.png" style="cursor:pointer;" width="16" height="16" onclick="formRemoveColumns(\''.$Row.'\');" />';	
					}
				$Form .= '</div>';
...
function formSetup_AddRow(){
	...
	jQuery('#formGridform').append('<div class="rowWrapperForm"><div style="clear:both; width:90%; float:left;" class="formRow" id="row'+rownum+'"><div style="padding:0; margin:0; width:100%; float:left;" id="row'+rownum+'_col1" class="column"><div class="ui-state-error formGridform formColumn" style="padding:10px; margin:10px;"></div></div></div><div style="width:10%; padding-top:12px; float:left;" class="formRow" id="row1Control"><img src="<?php echo WP_PLUGIN_URL; ?>/dbtoolkit/data_report/cog.png" style="cursor:pointer;" width="16" height="16" onclick="formSetupColumns(\'row'+rownum+'\');" /><img src="<?php echo WP_PLUGIN_URL; ?>/dbtoolkit/data_report/add.png" style="cursor:pointer;" width="16" height="16" onclick="formAddColumn(\'row'+rownum+'\');" /><img src="<?php echo WP_PLUGIN_URL; ?>/dbtoolkit/data_report/delete.png" style="cursor:pointer;" width="16" height="16" onclick="formSubtractColumn(\'row'+rownum+'\');" /><img src="<?php echo WP_PLUGIN_URL; ?>/dbtoolkit/data_report/no.png" style="cursor:pointer;" width="16" height="16" onclick="formRemoveColumns(\'row'+rownum+'\');" /></div></div>');
		jQuery(".formGridform").sortable({
			connectWith: '.formGridform',
			update: function(event, ui){
				jQuery(this).find(".positioning").val(jQuery(this).parent().attr('id')+'_'+jQuery(this).parent().css('width'));
