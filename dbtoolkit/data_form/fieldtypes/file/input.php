...
if($FieldSet[1] == 'multi'){
	...
	if(!empty($Defaults[$Field])){
		...
		foreach($FileList as $File){
			...
			$Return .= '<div class="cancel"><img border="0" class="togglebutton" src="'.WP_PLUGIN_DIR.'/dbtoolkit/data_form/fieldtypes/file/icons/cancel.png" style="cursor:pointer;" onclick="file_removeFile(\'entry_'.$Element['ID'].'_'.$Field.'_'.$entryID.'\');""><img border="0" class="togglebutton" src="'.WP_PLUGIN_DIR.'/dbtoolkit/data_form/fieldtypes/file/icons/add.png" style="cursor:pointer; display:none;" onclick="file_undoRemoveFile(\'entry_'.$Element['ID'].'_'.$Field.'_'.$entryID.'\');""></div>';
	...
	jQuery(\"#entry_".$Element['ID']."_".$Field."\").uploadify({
		'uploader'       : WP_PLUGIN_DIR.'/dbtoolkit/data_form/fieldtypes/file/scripts/uploadify.swf',
		'script'		 : '?uploadify=".urlencode(base64_encode($Req.'_'.$Element['ID'].'_'.$Field))."',
		'cancelImg'      : WP_PLUGIN_DIR.'/dbtoolkit/data_form/fieldtypes/file/icons/cancel.png',
		...
		'width'			 : '83',
		'height'		 : '21',
		'buttonImg'      : WP_PLUGIN_DIR.'/dbtoolkit/data_form/fieldtypes/file/icons/select.gif',
		'wmode'          : 'transparent',
